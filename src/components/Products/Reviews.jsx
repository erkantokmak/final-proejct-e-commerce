...
import { AllReviewCount, AllReviewText, StyledBox, StyledIcon, StyledSvg } from '@/styles/CommentStyle'

import WriteCommentModal from '../Comments/WriteCommentModal'
    ...
    return (
        <>
            <div className="d-flex justify-content-between">
                <AllReviewText className='mt-4'>
                    {t('allReviews')}<AllReviewCount>{'('}{data.comments.length}{')'}</AllReviewCount>
                </AllReviewText>
                <div className="d-flex align-items-center gap-3">
                    <StyledBox>
                        <StyledIcon>
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="contain">
                    ...
                    <WriteCommentModal data={data}/>
                </div>
            </div>
            <div className='d-none d-md-block'>
                <div className="row py-3">
                    {
                        data.comments.length > 0 && data.comments.slice(0, 6).map(
                            (comment, index) => (
                                <ProductComment key={index} data={comment} />
                            )
                        )
                    }
                    {
            ...
            <div className='d-block d-md-none'>
                <div className="row py-5">
                    {
                        data.comments.length > 0 && data.comments.slice(0, 3).map(
                            (comment, index) => (
                                <ProductComment key={index} data={comment} />
                            )
                        )
